...
export default function TransactionHistory1({ title, type, username }: TransactionHistoryProps) {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        const response = await axios.get(`http://localhost:5001/transactions/${username}`)
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{type === "deposit" ? "Deposited" : "Amount"}</TableHead>
